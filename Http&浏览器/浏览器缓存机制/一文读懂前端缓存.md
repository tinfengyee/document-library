...
[easonyq-github](https://github.com/easonyq/easonyq.github.io)

*[掘金版本](https://juejin.cn/post/6844903747357769742)*

## 前端缓存/后端缓存

扯了些没用的，我们先进入定义环节：什么是前端缓存？与之相对的什么又是后端缓存？

![request&response](./一文读懂前端缓存.assets/167e8739bb344367tplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293053857-3.png)
...
![图片](./一文读懂前端缓存.assets/640-1667295700964-51.jpeg)
...
![cache-control](./一文读懂前端缓存.assets/167e8739bc724fcbtplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293323716-7.png)
...
<u>当强制缓存失效(超过规定时间)时，就需要使用对比缓存</u>，由服务器决定缓存内容是否失效。
...
1. 首次请求
    ...
    ![First request](./一文读懂前端缓存.assets/167e8739bc1fe97btplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293846041-11.png)

    毫无意外的全部走网络请求，因为什么缓存都还没有。

2. 再次请求 (F5)

    ![Refresh](./一文读懂前端缓存.assets/167e8739bae094betplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293855104-15.png)

    第二次请求，三个请求都来自 memory cache。因为我们没有关闭 TAB，所以浏览器把缓存的应用加到了 memory cache。(耗时 0ms，也就是 1ms 以内)

3. 关闭 TAB，打开新 TAB 并再次请求

    ![Reopen](./一文读懂前端缓存.assets/167e8739bd27c2datplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293864741-19.png)
...
1. 当把服务器响应设置为 `Cache-Control: no-cache` 时，我们发现打开页面之后，三种资源都只被请求 __1__ 次。
    ...
    ![只请求一次](./一文读懂前端缓存.assets/167e8739bc1fe97btplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293879031-23.png)

    ![只请求一次](./一文读懂前端缓存.assets/167e8739bd65e62etplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293888950-27.png)
...
2. 当把服务器响应设置为 `Cache-Control: no-store` 时，情况发生了变化，三种资源都被请求了 __2__ 次。而图片因为还多一次异步请求，总计 __3__ 次。(红框中的都是那一次异步请求)
    ...
    ![请求多次](./一文读懂前端缓存.assets/167e8739ed290f92tplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293907525-31.png)

    ![请求多次](./一文读懂前端缓存.assets/167e8739ed347f62tplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293918161-35.png)
...
1. 当我们首次访问时，会看到常规请求之外，浏览器(确切地说是 Service Worker)额外发出了 3 个请求。这来自预缓存的代码。
    ...
    ![预缓存](./一文读懂前端缓存.assets/167e8739ed4c3418tplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293931284-39.png)

2. 第二次访问(无论关闭 TAB 重新打开，还是直接按 F5 刷新)都能看到所有的请求标记为 `from SerciceWorker`。

    ![缓存命中](./一文读懂前端缓存.assets/167e873a06d5b961tplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293938298-43.png)
    ...
    ![Service Worker 服务器](./一文读懂前端缓存.assets/167e873a0a2c1129tplv-t2oaga2asx-zoom-in-crop-mark4536000-1667293948694-47.png)
